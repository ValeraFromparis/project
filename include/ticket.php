...
class ticket {
     ...
     * Adds the ticket to the database
     ...
     if($_SESSION['usertype']=='Administrator'){
		...
		$result = $mysqli->query($sql);
		echo '<table class="table"><th>Клиент#</th><th>Комментарии</th><th>Назначение</th><th>Отдел</th><th>Статус|Действие</th>';
		if ($result->num_rows > 0) {
			// output data of each row
  ...
  Refer
			...
			echo "Нет запроса с таким статусом: " . $status;
		}
			echo '</table>';
			$mysqli->close();
 ...
 if($_SESSION['usertype']=='Administrator'){
	$sql = "select
		...
		$result = $mysqli->query($sql);
		echo '<table class="table"><th>Клиент#</th><th>Тема</th><th>Категория</th><th>Подкатегория</th><th>Назначение</th><th>Статус	</th>';
		if ($result->num_rows > 0) {
			// output data of each row
			while($row = $result->fetch_assoc()) {
			...
			echo "Введите ClientId, чтобы увидеть последние заявки.";
		}
		echo '</table>';
		$mysqli->close();
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Обратится в отдел </h5>
      ...
      <div class="modal-body"> 
      	...
      	<div class="form-group">
            <label for="group" class="control-label">Отдел :</label> 
    		<select class="form-control" name="groupid"> 
				<?php department::displayDepartmentsOptionList(); ?> 
    		</select> 
          </div>
            <div class="form-group">
                    <label for="comments" class="control-label">Примечания</label>
                    <div>
      ...
      <div class="modal-footer">
        ...
        <button type="submit" name="btnrefermodal" class="btn btn-primary">Сохранить изменения</button>
      </div>
 	 </form>
    </div>
  </div>
